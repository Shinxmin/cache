...
// 휴지통 화면의 전체 삭제·전체 복원·개별 삭제·개별 복원에 쓰는 공용 확인
// 모달. 다른 모달과 같은 카드 껍데기를 쓰지만 목록·입력창 없이 안내 문구
// (필요하면 그 밑에 작은 경고 문구 한 줄) + 확인 버튼뿐이다. 배경을 탭하거나
// 제목 줄 우측의 x를 누르면 취소된다.
//
...
export default function ConfirmModal({ title, message, warning, onClose, onSubmit }) {
  useBodyScrollLock();
  ...
  return (
    <div className={`rename-overlay confirm-modal-overlay${closing ? " is-closing" : ""}`} onClick={requestClose}>
