...
// 스튜디오 클립 섹션의 구간 슬라이더. 시작·끝 두 손잡이를 각각 드래그하거나,
// 트랙의 빈 자리를 누르면 더 가까운 쪽 손잡이가 그 위치로 옮겨간다. 실제
// 0.5초 최소 간격 제한이나 duration 경계 클램프는 App.jsx의
// changeStudioClipStart/End가 맡는다 — 여기서는 누른 지점의 초(sec)값만
// 계산해 그대로 올려보낸다.
export default function ClipRangeSlider({ duration, start, end, onChangeStart, onChangeEnd, disabled }) {
  ...
  return (
    <div
      className={`search-bar-clip-slider${disabled ? " is-disabled" : ""}`}
      ref={trackRef}
    ...
    >
      <div className="search-bar-clip-slider-track" />
      <div
        className="search-bar-clip-slider-range"
        style={{ left: `${startPct}%`, width: `${Math.max(0, endPct - startPct)}%` }}
      />
      <button
        type="button"
        className="search-bar-clip-slider-thumb"
        aria-label="시작 지점"
        style={{ left: `${startPct}%` }}
