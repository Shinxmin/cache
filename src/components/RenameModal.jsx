...
//    (모든 입력창을 비운다)·"전체 적용"(1번째 입력창 값을 모두에게 적용,
//    이름은 중복될 수 없어 자동으로 (1),(2),(3)…을 붙인다)·"번호 붙이기"
//    (1번째 값 끝의 숫자를 기준점 삼아 순서대로 번호를 잇는다) + 확인 버튼.
export default function RenameModal({ items, onClose, onSubmit }) {
  ...
  // 1번째 입력창 값을 모두에게 그대로 적용한다. 다만 이름은 중복될 수 없으므로
  // (전부 똑같은 이름이 되어 버리니) 자동으로 뒤에 (1),(2),(3)…을 붙인다.
  const applyToAll = () =>
    setNames((prev) => {
      const base = prev[0] || "";
      return prev.map((_, i) => `${base}(${i + 1})`);
    });
